...
S_INLINE void screen_dirty_row(int y)
{
	if (0 <= y && y < SCREEN_H) {
...
S_INLINE int screen_dirty_test(int y)
{
	return (screen_dirty_rows & (1u << y)) != 0;
}

S_INLINE void screen_dirty_clear_row(int y)
{
	screen_dirty_rows &= ~(1u << y);
}

S_INLINE void screen_dirty_clear_all(void)
{
	screen_dirty_rows = 0;
}
#else
